...
constexpr int num_node = 8;

namespace CGL {

Application::Application(AppConfig config) : config(config) {}
...
void Application::init() {
  ...
  // Create two ropes 
  ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), num_node, config.mass, config.ks, {0});
  ropeVerlet = new Rope(Vector2D(0, 200), Vector2D(-400, 200), num_node, config.mass, config.ks, {0});
}
...
void Application::render()
{
  ...
  {
    ropeEuler->simulateEuler(1.F / config.steps_per_frame, config.gravity);
    ropeVerlet->simulateVerlet(1.F / config.steps_per_frame, config.gravity);
  }
  // Rendering ropes
  Rope *rope;
