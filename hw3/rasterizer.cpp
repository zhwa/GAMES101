...
void rst::rasterizer::draw(std::vector<Triangle*>& TriangleList)
{
    ...
    for (const auto& t : TriangleList)
    {
        ...
        std::ranges::transform(mm, viewspace_pos.begin(), [](auto& v) { return v.head<3>(); });
            ...
            if (x >= x_min && x <= x_max && y >= y_min && y <= y_max)
            {
                ...
                if (insideTriangle(x, y, t.v))
                {
                    ...
                    if (depth_buf[ind] > z_interpolated)
                    {
                        ...
                        // Unlike HW2, where you only need to do: set_pixel(point, t.getColor());
                        // Here, the following have to be calcuated:
                        ...
                        Eigen::Vector3f interpoloated_shadingcoords = alpha * view_pos[0] + beta * view_pos[1] + gamma * view_pos[2];
                        ...
                        payload.view_pos = interpoloated_shadingcoords;
                        const auto pixel_color = fragment_shader(payload);
