    ...
    if(intersection.happened)
    {
        ...
        switch (m->getType())
        {
            case MaterialType::REFLECTION_AND_REFRACTION:
            {
                ...
                Vector3f reflectionRayOrig = (dotProduct(reflectionDirection, N) < 0) ? hitPoint - N * std::numeric_limits<float>::epsilon() : hitPoint + N * std::numeric_limits<float>::epsilon();
                Vector3f refractionRayOrig = (dotProduct(refractionDirection, N) < 0) ? hitPoint - N * std::numeric_limits<float>::epsilon() : hitPoint + N * std::numeric_limits<float>::epsilon();
            ...
            default:
            {
                ...
                Vector3f shadowPointOrig = (dotProduct(ray.direction, N) < 0) ? hitPoint + N * std::numeric_limits<float>::epsilon() : hitPoint - N * std::numeric_limits<float>::epsilon();
                // [comment]
