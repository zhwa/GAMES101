...
void Renderer::Render(const Scene& scene)
{
    ...
    for (auto i = 0; i < scene.height * scene.width; ++i) {
        ...
        color[0] = (unsigned char)(255 * std::clamp<float>(framebuffer[i].x, 0.F, 1.F));
        color[1] = (unsigned char)(255 * std::clamp<float>(framebuffer[i].y, 0.F, 1.F));
        color[2] = (unsigned char)(255 * std::clamp<float>(framebuffer[i].z, 0.F, 1.F));
        fwrite(color, 1, 3, fp);
    }
    fclose(fp);    
}