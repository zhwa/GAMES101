...
{
    float cosi = std::clamp<float>(dotProduct(I, N), -1, 1);
...
void Renderer::Render(const Scene& scene)
{
    ...
    for (auto i = 0; i < scene.height * scene.width; ++i)
    {
        ...
        color[0] = static_cast<char>(255 * std::clamp<float>(framebuffer[i].x, 0, 1));
        color[1] = static_cast<char>(255 * std::clamp<float>(framebuffer[i].y, 0, 1));
        color[2] = static_cast<char>(255 * std::clamp<float>(framebuffer[i].z, 0, 1));
        fwrite(color, 1, 3, fp);
    }
    fclose(fp);
}